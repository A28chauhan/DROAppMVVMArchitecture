package com.example.folding_cell;
...
import com.example.folding_cell.animations.AnimationEndListener;
import com.example.folding_cell.animations.FoldAnimation;
import com.example.folding_cell.animations.HeightAnimation;
import com.example.folding_cell.views.FoldingCellView;
...
public class FoldingCell extends RelativeLayout{
    ...
    // default values
    private final int DEF_ANIMATION_DURATION = 700;
    private final int DEF_BACK_SIDE_COLOR = Color.LTGRAY;
     ...
     */
    public void unfold(boolean skipAnimation,int screenHeight) {
        ...
        Bitmap bitmapFromTitleView = measureViewAndGetBitmap(titleView, this.getMeasuredWidth(),screenHeight);
        Bitmap bitmapFromContentView = measureViewAndGetBitmap(contentView, this.getMeasuredWidth(),screenHeight);
     ...
     * @param skipAnimation if true - change state of cell instantly without animation
     */
    public void fold(boolean skipAnimation,int screenHeight) {
        if (!mUnfolded || mAnimationInProgress) return;
     ...
     */
    public void toggle(boolean skipAnimation,int screenHeight) {
        if (this.mUnfolded) {
            this.fold(skipAnimation,screenHeight);
        } else {
            this.unfold(skipAnimation,screenHeight);
            this.requestLayout();
        }
    }
    ...
    protected ArrayList<Integer> calculateHeightsForAnimationParts(int titleViewHeight, int contentViewHeight, int additionalFlipsCount) {
        ...
        if(additionalPartsTotalHeight < 0){
            additionalPartsTotalHeight= contentViewHeight - titleViewHeight * 1;
        }
     ...
     */
    protected Bitmap measureViewAndGetBitmap(View view, int parentWidth,int screenHeight) {
        Bitmap b = null;
        try {
            ...
            int specH = MeasureSpec.makeMeasureSpec(screenHeight, MeasureSpec.AT_MOST);
